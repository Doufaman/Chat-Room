...
# Legacy message types from original system
TYPE_WHO_IS_LEADER = 'WHO_IS_LEADER'
TYPE_I_AM_LEADER = 'I_AM_LEADER'
TYPE_JOIN_SERVER = 'JOIN_SERVER'
TYPE_LOAD_UPDATE = 'LOAD_UPDATE'

def set_server_message(msg_type, sender, content=None):
...
def set_client_message(msg_type, sender, vector_clock, content=None):
    """generate the message in json format"""
    message = {
        'type': msg_type,
        'sender': sender, 
        'vector_clock': vector_clock.copy() if hasattr(vector_clock, 'copy') else vector_clock, 
        'content': content if content else {}
    }
    return message
